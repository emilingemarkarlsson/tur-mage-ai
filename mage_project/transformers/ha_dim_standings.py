...
@transformer
def ha_dim_standings(data, *args, **kwargs):
    """
    Steg 4/4 – Aggregerar analytics.fact_team_game → analytics.dim_standings.
    Kräver att ha_fact_team_game körts först (upstream-block).
    """
    conn = _connect()
    try:
        conn.execute(f"""
        ...
        print(f"[ha] dim_standings: {count} rader skrivna")
        return {"dim_standings": count}
    except Exception as exc:
        print(f"[ha] ERROR dim_standings: {exc}")
        raise
    finally:
        conn.close()
